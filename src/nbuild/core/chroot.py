...
from nbuild.core import logger, paths, errors
from nbuild.apk import cli as apki, sign as signi
...
def chroot_check(force=False, error=True):
    ...
    if (paths.bldroot() / ".nbuild_chroot_init").is_file():
        _chroot_ready = True
        cpun = (paths.bldroot() / ".nbuild_chroot_init").read_text().strip()
        set_host(cpun)
    else:
        _chroot_ready = False
...
def _prepare_etc():
    ...
    with open(tfp / "passwd", "a") as pf:
        pf.write("nbuild:x:1337:1337:nbuild user:/tmp:/bin/nologin\n")

    with open(tfp / "group", "a") as pf:
        pf.write("nbuild:x:1337:\n")

    # machine-id for nbuild
    ...
    # delete potential previous nbuild file so we are clean
    (paths.bldroot() / "usr/lib/sysusers.d/nbuild.conf").unlink(missing_ok=True)
...
def _prepare():
    sfpath = paths.bldroot() / ".nbuild_chroot_init"
    if sfpath.is_file():
        return
    if not (paths.bldroot() / "usr" / "bin" / "sh").is_file():
        raise errors.NbuildException("bootstrap not installed, can't continue")
...
def shell_update(rnet, dirty):
    ...
    with rfile.open("w") as rfh:
        ...
        if rnet:
            from nbuild.core import profile
    ...
    (paths.nbuild_cache() / "apk" / hcpu).mkdir(parents=True, exist_ok=True)
    cfile.symlink_to(f"/nbuild_cache/apk/{hcpu}")
    ...
    with flock.lock(flock.apklock(hcpu)):
        if (
            ...
            raise errors.NbuildException("failed to update pkg database")
...
def install():
    ...
    from nbuild.core import template

    logger.get().out(f"nbuild: installing {' '.join(_extra_pkgs)}...")
    ...
    if irun.returncode != 0:
        raise errors.NbuildException("failed to install container packages")

    logger.get().out("nbuild: container set up successfully!")
...
def get_fakeroot(bootstrap):
    inp = paths.nbuild() / "misc/fakeroot.sh"

    if bootstrap:
        return inp

    rp = paths.bldroot() / ".nbuild_fakeroot.sh"

    if rp.is_file():
        return "/.nbuild_fakeroot.sh"
...
def _setup_dummy(rootp, archn):
    ...
    logger.get().out(f"nbuild: installing virtual provider for {archn}...")
...
def _prepare_arch(prof, dirty):
    ...
    if rootp.exists() and not dirty:
        logger.get().out(f"nbuild: clearing sysroot for {prof.arch}...")
        shutil.rmtree(rootp)
...
def cleanup_world(bootstrapping, prof=None, perform=True):
    ...
    if bootstrapping is None:
        bootstrapping = not (paths.bldroot() / ".nbuild_chroot_init").is_file()

    log = logger.get()

    log.out("nbuild: cleaning world...")
    ...
    if f_ret.returncode != 0:
        raise errors.NbuildException("failed to remove autodeps")
...
def update(pkg):
    ...
    logger.get().out(
        f"nbuild: updating software in {paths.bldroot()!s} container..."
    )
    ...
    with flock.lock(flock.apklock(prof.arch)):
        if (
            ...
            raise errors.NbuildException("failed to update cross pkg database")
...
def enter(
    ...
    *args,
    ...
    envs = {
        ...
        "SHELL": "/bin/sh",
        "HOME": "/tmp",
        "USER": "root" if fakeroot else "nbuild",
        "LC_COLLATE": "C",
        "LANG": "C.UTF-8",
        **env,
    }
    ...
    if "NBUILD_STATEDIR" in envs:
        envs["PATH"] = envs["NBUILD_STATEDIR"] + "/wrappers:" + envs["PATH"]
    ...
    if mount_nbuild_cache:
        bcmd += ["--bind", paths.nbuild_cache(), "/nbuild_cache"]

    # always bubblewrap as nbuild user
    ...
    if lldargs:
        ...
        os.close(wfd)
        fdlist.append(rfd)
        bcmd += ["--ro-bind-data", str(rfd), "/tmp/nbuild-lld-args"]
