...
from nbuild.core import logger, chroot, paths, profile, spdx, errors
from nbuild.util import compiler, flock
from nbuild.apk import cli, util as autil
...
class Template(Package):
    ...
    def init_from_mod(self):
        ...
        if self.build_style:
            importlib.import_module(
                f"nbuild.build_style.{self.build_style}"
            ).use(self)
        ...
        if self.broken:
            ...
            self.broken = f"cannot be built, disallowed by nbuild (not in {', '.join(_allow_cats)})"
    ...
    def get_build_deps(self):
        from nbuild.core import dependencies
    ...
    def setup_paths(self):
        ...
        self.statedir = bdirbase / (".nbuild-" + self.pkgname)
        ...
        self.env["NBUILD_STATEDIR"] = "/builddir/.nbuild-" + self.pkgname
        ...
        for sp in self.subpkg_list:
            ...
            if not tpath.is_symlink():
                self.error(
                    ...
                    hint="run 'nbuild relink-subpkgs' to rebuild them",
                )
    ...
    def do(
        ...
        *args,
        ...
        cenv = {
            "NBUILD_TARGET_MACHINE": cpf.arch,
            "NBUILD_TARGET_SYSROOT": str(cpf.sysroot),
            "NBUILD_HOST_MACHINE": chroot.host_cpu(),
        }
        ...
        if cpf.triplet:
            cenv["NBUILD_TARGET_TRIPLET"] = cpf.triplet

        if self.use_ccache:
            cenv["CCACHEPATH"] = "/usr/lib/ccache/bin"
            cenv["CCACHE_DIR"] = "/nbuild_cache/ccache"
        ...
        if (
            ...
            cenv["SCCACHE_DIR"] = "/nbuild_cache/sccache"
        ...
        with self.profile("host") as hpf:
            ...
            if hpf.triplet:
                cenv["NBUILD_HOST_TRIPLET"] = hpf.triplet
        ...
        if self.use_ltocache:
            lld_args += [
                ...
                "--thinlto-cache-dir=/nbuild_cache/lld_thinlto_cache",
            ]
...
def sanitize_pkgname(pkgname, error=True):
    ...
    if not tmplpath.is_file():
        if not error:
            return None
        raise errors.NbuildException(f"missing template for '{pkgname}'")
